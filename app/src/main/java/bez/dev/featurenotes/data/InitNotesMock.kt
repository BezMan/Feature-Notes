...
class InitNotesMock(private val noteRepository: NoteRepository) : INotesInit {
    ...
    override suspend fun setInitNotes() {
        if (SharedPrefs.getBoolValue(KEY_FIRST_RUN, true)) {
            ...
            for (i in 1..initItems) {
                noteRepository.insert(Note("mock $i", i, mutableListOf(NoteItem("select edit"), NoteItem("drag and drop"))))
            }
